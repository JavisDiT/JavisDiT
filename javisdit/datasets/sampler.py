...
from javisdit.utils.misc import format_numel_str, get_logger

from .aspect import get_num_pixels
from .bucket import Bucket
from .datasets import VariableVideoTextDataset, VariableVideoAudioTextDataset
...
class VariableVideoBatchSampler(DistributedSampler):
    def __init__(
        ...
        last_micro_batch_access_index: int = 0,
    ) -> None:
        super().__init__(
        ...
        self.last_micro_batch_access_index = last_micro_batch_access_index
    ...
    def __iter__(self) -> Iterator[List[int]]:
        # bucket_sample_dict: {key = (res_type, frame_num, aspect_ratio); value = List[item index]}
        ...
        bucket_micro_batch_count = OrderedDict()  # {key = (res_type, frame_num, aspect_ratio); value = batch_cnt}
        bucket_last_consumed = OrderedDict()  # {key = (res_type, frame_num, aspect_ratio); value = sample_cnt}

        # process the samples
        for bucket_id, data_list in bucket_sample_dict.items():
            # handle droplast
            bs_per_gpu = self.bucket.get_batch_size(bucket_id)  # pre-defined in config
            ...
            num_micro_batches = len(data_list) // bs_per_gpu  # already divisible
        ...
        for i in range(start_iter_idx, num_iters):
            ...
            # passed new `index` to `dataset.__getitem__(index)`
    ...
    def group_by_bucket(self) -> dict:
        ...
        )  # List[res_type, frame_num, aspect_ratio]
    ...
    def get_num_batch(self) -> int:
        bucket_sample_dict = self.group_by_bucket()
        self._get_num_batch_cached_bucket_sample_dict = deepcopy(bucket_sample_dict)
...
class VariableVideoAudioBatchSampler(VariableVideoBatchSampler):
    def __init__(
        self,
        dataset: VariableVideoAudioTextDataset,
        bucket_config: dict,
        num_replicas: Optional[int] = None,
        rank: Optional[int] = None,
        shuffle: bool = True,
        ...
        super().__init__(
            dataset=dataset, bucket_config=bucket_config, num_replicas=num_replicas, rank=rank, 
            shuffle=shuffle, seed=seed, drop_last=drop_last, verbose=verbose, 
            num_bucket_build_workers=num_bucket_build_workers, last_micro_batch_access_index=last_micro_batch_access_index
        )
