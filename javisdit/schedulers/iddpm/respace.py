...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def multimodal_training_losses(self, model, *args, **kwargs):  # pylint: disable=signature-differs
        if isinstance(model, dict):
            model = {k: self._wrap_model(m) for k, m in model.items()}
        else:
            model = self._wrap_model(model)
        return super().multimodal_training_losses(model, *args, **kwargs)
