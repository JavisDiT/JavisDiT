...
def mean_flat(tensor: torch.Tensor, mask=None, eps=1e-6):
    """
    ...
    if mask is None:
        ...
        elif tensor.dim() == 4: # audio
            tensor = rearrange(tensor, "b c t s -> b t (c s)")
        else:
            raise NotImplementedError(f'Unrecognized tensor dim: {tensor.dim()}')
        numer = (tensor * mask.unsqueeze(2)).sum(dim=1).sum(dim=1) + eps
        denom = mask.sum(dim=1) * tensor.shape[-1] + eps
        loss = numer / denom
        return loss
...
class GaussianDiffusion:
    ...
    def multimodal_training_losses(self, model, x_start: Dict[str, torch.Tensor], model_kwargs=None, 
                                   noise=None, mask=None, weights=None, t=None):  # , modal_key=None
        """
        Compute multimodal training losses for a single timestep.
        :param model: the model to evaluate loss on.
        :param x_start: the {modal: tensor[N x C x ...] } dict of inputs.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param noise: if specified, the specific Gaussian noise to try to remove.
        :return: a dict with the key "loss" containing a tensor of shape [N].
        ...
        bs, device = x_start[modal_keys[0]].shape[0], x_start[modal_keys[0]].device
        ...
        for k in modal_keys:
            ...
            x_t[k] = self.q_sample(x_start[k], t, noise=noise[k])
            if k == 'video' and mask is not None:  # TODO: audio
                t0 = torch.zeros_like(t)
                x_t0 = self.q_sample(x_start[k], t0, noise=noise[k])
                x_t[k] = torch.where(mask[:, None, :, None, None], x_t[k], x_t0)

        if model_kwargs is None:
            model_kwargs = {}
        # TODO: specialized for VALDM2
        model_kwargs.update({
            'num_timesteps': self.num_timesteps,  # for vx_start prediction
            'vx_pred': x_start['video'],          # vx_start ground-truth
            'return_tensor': True,                # compatible for iddpm
        })
        x_start, x_t, noise = x_start['audio'], x_t['audio'], noise['audio']
        mask = None
