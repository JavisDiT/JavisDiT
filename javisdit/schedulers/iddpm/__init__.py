...
class IDDPM(SpacedDiffusion):
    ...
    def audio_sample(
        ...
        dtype = z.dtype
        model_args = {}
        ay, audio_mask, ay2 = model.encode_audio_prompt(
            prompts, 
            do_classifier_free_guidance=True
        )
        model_args.update({
            "ay": ay.to(device, dtype),
            "audio_mask": audio_mask.to(device, dtype),
            "ay2": ay2.to(device, dtype),
        })
        model_args.update({"y": model_args["ay"], "x_mask": model_args["audio_mask"]})  # compatible
        if additional_args is not None:
            model_args.update(additional_args)
        forward = partial(forward_with_cfg, model, cfg_scale=self.cfg_scale, cfg_channel=self.cfg_channel)
        samples: torch.Tensor = self.p_sample_loop(
            forward,
            z.shape,
            z,
            clip_denoised=False,
        ...
        if samples.shape[1] != z.shape[1]:
            assert samples.shape[1] == z.shape[1] * 2
            samples = samples.chunk(2, dim=1)  # mean, var
        
        samples, _ = samples.chunk(2, dim=0)  # classifier-free guidance
...
def forward_with_cfg(model, x, timestep, y, cfg_scale, cfg_channel=None, **kwargs):
    ...
    if cfg_channel is None:
        if model_out.shape[1] != x.shape[1]:
            assert model_out.shape[1] == x.shape[1] * 2, f'{model_out.shape} {x.shape}'
        cfg_channel = x.shape[1]
