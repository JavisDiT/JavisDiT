...
def get_model_numel(model: torch.nn.Module) -> Tuple[int, int]:
    ...
    for name, p in model.named_parameters():
        # print(p.requires_grad, name, format_numel_str(p.numel()))
        num_params += p.numel()
        if p.requires_grad:
...
def check_exist_pickle(data, path):
    if not os.path.exists(path):
        return
    buffer = torch.load(path)
    assert type(data) == type(buffer)
    if isinstance(buffer, dict):
        for k, v in buffer.items():
            assert k in data
            s = data[k]
            assert type(s) == type(v)
...
class Timer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.log:
            print(f"Elapsed time for {self.name}: {self.elapsed_time*1000:.2f} ms")
...
class FeatureSaver:
    ...
    def save(self):
        ...
        if os.path.exists(save_path):
            exist_data_list = torch.load(save_path)
            assert len(exist_data_list) == len(self.data_list)
            for dst, src in zip(exist_data_list, self.data_list):
                for k, v in src.items():
                    if k in dst:
                        if isinstance(v, torch.Tensor):
                            assert v.shape == dst[k].shape, f'{k}, {v.shape}, {dst[k].shape}'
                            assert (v == dst[k]).all(), f'{k}, {v}, {dst[k]}'
                        else:
                            assert v == dst[k]
                    else:
                        dst[k] = v
            self.data_list = exist_data_list
        # else:
        #     exit()
    ...
    def get_num_saved(self, check_keys=()):
        for path in tqdm(sorted(glob(os.path.join(self.save_dir, '*.bin'))), desc='Scaning existing data'):
            data = torch.load(path)
            if len(data) != self.bin_size:
                s = f'Saved bin_size {len(data)} mismatch defined {self.bin_size}. Start at {self.bin_cnt}.'
                get_logger().info(s)
                break
            processed = True
            for item in data:
                if any(key not in item for key in check_keys):
                    processed = False
                    break
            if not processed:
                s = f'Saved {path} lack required keys {check_keys}. Start at {self.bin_cnt}.'
                get_logger().info(s)
                break
            self.cnt += self.bin_size
            self.bin_cnt += 1
