...
from javisdit.acceleration.parallel_states import set_data_parallel_group, set_sequence_parallel_group
from javisdit.acceleration.plugin import ZeroSeqParallelPlugin
...
class MaskGenerator:
    ...
    def get_mask(self, x, require_name=False):
        ...
        if mask_name == "quarter_random":
            ...
            if not mask.any():
                mask[-1] = 1
        elif mask_name == "video_to_audio":
            mask[:] = 0
        elif mask_name == "audio_to_video":
            mask[:] = 1
        elif mask_name == "sound_image_animate":
            mask[:1] = 0
...
class VAMaskGenerator(MaskGenerator):
    def __init__(self, mask_ratios):
        super().__init__(mask_ratios)

    def get_masks(self, vx, ax):
        vx_masks, mask_names = [], []
        for _ in range(len(vx)):
            mask, mask_name = self.get_mask(vx, require_name=True)
            vx_masks.append(mask)
            mask_names.append(mask_name)
        vx_masks = torch.stack(vx_masks, dim=0)
        
        ax_masks = self.get_audio_masks(vx_masks, ax, mask_names)

        return vx_masks, ax_masks

    def get_audio_masks(self, vx_masks: torch.Tensor, ax: torch.Tensor, mask_names: List[str]):
        """inputs
            vx_masks: video temporal mask, shape (B, Nv)
            ax: audio latent representation, shape (B, 1, T, S)
            mask_names: masking strategies
        """
        # ax_mask = F.interpolate(vx_masks[None].float(), size=ax.shape[2], mode='linear', align_corners=True)
        # ax_mask = ax_mask[0] > 0.5
        ax_mask = F.interpolate(vx_masks[None].float(), size=ax.shape[2], mode='nearest')
        ax_mask = ax_mask[0] > 0.

        # 0=data, 1=noise
        assert len(mask_names) == len(ax_mask)
        for i, mask_name in enumerate(mask_names):
            if mask_name == "sound_image_animate":
                ax_mask[i, :] = 0
            elif "image" in mask_name:
                ax_mask[i, :] = 1
            elif mask_name == "audio_to_video":
                ax_mask[i, :] = 0
            elif mask_name == "video_to_audio":
