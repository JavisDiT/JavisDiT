...
def parse_args(training=False):
    ...
    parser.add_argument("--model-path", default=None, type=str, help="path to load model weights")
    ...
    if not training:
        ...
        parser.add_argument("--audio-prompt-path", default=None, type=str, help="path to prompt txt file")
        parser.add_argument("--audio-prompt", default=None, type=str, nargs="+", help="prompt list")
        ...
        parser.add_argument("--audio-fps", default=None, type=int, help="fps")
    ...
    # ======================================================
        ...
        parser.add_argument("--port", default=29500, type=int, help="port number for multi-node training")
...
def merge_args(cfg, args, training=False):
    ...
    if args.flash_attn is not None:
        ...
        if cfg.get("prior_encoder", None) is not None:
            cfg.prior_encoder["enable_flash_attn"] = args.flash_attn
        args.enable_flash_attn = None
    if args.layernorm_kernel is not None:
        cfg.model["enable_layernorm_kernel"] = args.layernorm_kernel
        if cfg.get("prior_encoder", None) is not None:
            cfg.prior_encoder["enable_layernorm_kernel"] = args.layernorm_kernel
...
def define_experiment_workspace(cfg, get_last_workspace=False, model_name=None):
    """
