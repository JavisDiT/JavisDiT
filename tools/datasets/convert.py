...
def get_filelist(file_path, exts=None):
    ...
    # obj = glob(f'{file_path}/**/*.*', recursive=True)
    for entry in tqdm(obj, desc='Scaning'):
        if entry.is_file():
...
def process_general_images(root, output, difference=None):
    root = os.path.expanduser(root)
    ...
    if difference:
        data_diff = pd.read_csv(difference)
        print(f"Difference csv contains {len(data_diff)} samples.")
        df = df[~df["path"].isin(data_diff["path"])]
        print(f"Filtered number of samples: {len(df)}.")
...
def process_general_videos(root, output, difference=None):
    root = os.path.expanduser(root)
    if not os.path.exists(root):
        return
...
def process_general_audios(root, output, difference=None):
    root = os.path.expanduser(root)
    if not os.path.exists(root):
        return
    path_list = get_filelist(root, AUD_EXTENSIONS)
    path_list = list(set(path_list))  # remove duplicates
    fname_list = [os.path.splitext(os.path.basename(x))[0] for x in path_list]
    relpath_list = [os.path.relpath(x, root) for x in path_list]
    df = pd.DataFrame(dict(id=fname_list, relpath=relpath_list, audio_path=path_list))

    if difference:
        data_diff = pd.read_csv(difference)
        print(f"Difference csv contains {len(data_diff)} samples.")
        df = df[~df["audio_path"].isin(data_diff["audio_path"])]
        print(f"Filtered number of samples: {len(df)}.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("dataset", type=str, choices=["imagenet", "ucf101", "vidprom", "image", "video", "audio"])
    parser.add_argument("root", type=str)
    parser.add_argument("--split", type=str, default="train")
    parser.add_argument("--info", type=str, default=None)
    parser.add_argument("--output", type=str, default=None, required=True, help="Output path")
    parser.add_argument("--difference", type=str, default=None, help="get difference from existing dataset")
    ...
    if args.dataset == "imagenet":
        ...
        process_general_images(args.root, args.output, args.difference)
    elif args.dataset == "video":
        process_general_videos(args.root, args.output, args.difference)
    elif args.dataset == "audio":
        process_general_audios(args.root, args.output, args.difference)
    else:
